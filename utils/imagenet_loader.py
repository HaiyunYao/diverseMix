...
class ImageNet(torch.utils.data.Dataset):
    ...
    def __init__(self, path,transform=None, img_size=64,get_idx=False,use_rate=1,min_class=0, max_class=1000):
        ...
        for idx in range(1, 11):
            data_file = os.path.join(path, 'train_data_batch_{}'.format(idx))
        ...
        self.class_index2all_index = np.where((self.labels < max_class)*(self.labels>=min_class))[0]
    ...
    def load_image_batch(self, batch_index):
        ...
        data_file = os.path.join(self.path, 'train_data_batch_{}'.format(batch_index))
        d = unpickle(data_file)
        x = d['data']
    ...
    def __getitem__(self, index):
        ...
        index=self.class_index2all_index[index] 
